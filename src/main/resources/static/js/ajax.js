...
function ajaxSearch() {
    ...
    $.ajax({
        type: "POST",
        url: "/searchMusic",
        data: "pageSize=" + mkPlayer.loadcount + "&source=" + rem.source + "&pageIndex=" + rem.loadPage + "&name=" + rem.wd,
        dataType : "jsonp",
        ...
        success: function(jsonData){
            ...
            for (var i = 0; i < jsonData.length; i++) {
                ...
                tempItem =  {
                    ...
                    url: jsonData[i].url,   // mp3链接
                    lyric:jsonData[i].lyric     //歌词
                };
...
{
    // 已经有数据，直接回调
   if(music.url !== null && music.url !== "err" && music.url !== "" && music.source === "xiami") {
        callback(music);
        return true;
    }
    // id为空，赋值链接错误。直接回调
...
function ajaxPic(music, callback)
{
    ...
    $.ajax({ 
        type: "POST",
        url: "/loadPic",
        data: "id=" + music.pic_id + "&source=" + music.source,
        dataType : "jsonp",
        success: function(jsonData){
            // 调试信息输出
            if(mkPlayer.debug) {
...
function ajaxLyric(music, callback) {
    if(music.lyric !== null && music.lyric !== "err" && music.lyric !== "" && music.lyric != undefined) {
    ...
    $.ajax({
        type:"POST",
        url: "/loadLrc",
        data: "id=" + music.lyric_id + "&source=" + music.source,
        dataType : "jsonp",
        success: function(jsonData){
            // 调试信息输出
            if (mkPlayer.debug) {
            ...
            if (jsonData.lyric) {
                ...
                callback(jsonData.tlyric, music.lyric_id);    // 回调函数
            }
        },   //success
        error: function(XMLHttpRequest, textStatus, errorThrown) {
...
function ajaxKgUserList()
{
    $.ajax({
        ...
        dataType : "jsonp",
        success: function(jsonData){
            console.debug("全民K歌歌单获取成功" + jsonData );
            if (jsonData && jsonData.length > 0) {
                for(var i = 0; i < jsonData.length; i++) {
                    ajaxKgPlayList(jsonData[i])
                }
            }
        },   //success
    });//ajax
...
function ajaxKgPlayList(id){
    $.ajax({
        ...
        success: function(jsonData){
            ...
            if(typeof jsonData.item !== undefined || jsonData.item.length !== 0) {
                // 存储歌单中的音乐信息
